...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_search_sessions(self, request: "web.Request") -> "web.Response":
        ...
        results = self._get_session_db().search_messages(
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        session = self._normalize_session_record(
            self._get_session_db().get_session(session_id)
        )
        if session is None:
            return web.json_response({"error": "Session not found"}, status=404)
        return web.json_response({"session": session})

    async def _handle_get_session_messages(
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        if raw_attachments_sync:
            logger.debug(
                "[chat] Received %d attachment(s): %s",
                len(raw_attachments_sync),
                [
                    (
                        a.get("name"),
                        a.get("contentType"),
                        len(a.get("content", "") or a.get("base64", "") or ""),
                    )
                    for a in raw_attachments_sync
                    if isinstance(a, dict)
                ],
            )
        user_content, persist_text = self._build_user_content(
            message, raw_attachments_sync
        )
        if isinstance(user_content, list):
            logger.debug(
                "[chat] Built multimodal content with %d parts", len(user_content)
            )
        ...
        try:
            ...
            logger.error(
                "Error running session chat for %s: %s", session_id, e, exc_info=True
            )
            return web.json_response({"error": str(e)}, status=500)
    ...
    async def _handle_session_chat_stream(
        ...
        if raw_attachments:
            logger.debug(
                "[chat/stream] Received %d attachment(s): %s",
                len(raw_attachments),
                [
                    (
                        a.get("name"),
        ...
        if isinstance(user_content, list):
            logger.debug(
                "[chat/stream] Built multimodal content with %d parts",
                len(user_content),
            )
        ...
        try:
            ...
            await response.write(
                _encode_sse(
                    "done",
                    {"session_id": session_id, "run_id": run_id, "state": "final"},
                )
            )
            ...
            if not agent_task.done():
                ...
                try:
                    ...
                    pass
            logger.info(
                "Session SSE client disconnected; interrupted session %s", session_id
            )
    ...
    async def _handle_get_memory(self, request: "web.Request") -> "web.Response":
        ...
        if target not in {"all", "memory", "user"}:
            return web.json_response(
                {"error": "target must be one of: all, memory, user"}, status=400
            )
    ...
    async def _handle_add_memory(self, request: "web.Request") -> "web.Response":
        ...
        if target not in {"memory", "user"}:
            return web.json_response(
                {"error": "target must be 'memory' or 'user'"}, status=400
            )
    ...
    async def _handle_available_models(self, request: "web.Request") -> "web.Response":
        ...
        current = self._current_model_settings(config)
        provider = (
            request.query.get("provider") or current["provider"] or "openrouter"
        ).strip()
        models = [
            ...
            for model_id, desc in curated_models_for_provider(p["id"]):
                if model_id not in model_ids:
                    models.append(
                        {
                            "id": model_id,
                            "description": desc or p.get("label", p["id"]),
                            "provider": p["id"],
                        }
                    )
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app.router.add_get(
                "/api/sessions/search", self._handle_search_sessions
            )
            self._app.router.add_get(
                "/api/sessions/{session_id}", self._handle_get_session
            )
            self._app.router.add_get(
                "/api/sessions/{session_id}/messages", self._handle_get_session_messages
            )
            self._app.router.add_patch(
                "/api/sessions/{session_id}", self._handle_update_session
            )
            self._app.router.add_delete(
            ...
            self._app.router.add_get(
                "/api/skills/categories", self._handle_skill_categories
            )
